...
def main() -> None:
    ...
    search_rows = read_csv("fact_search_interest.csv")
    prophet_rows = read_csv("fact_forecast_prophet.csv")
    rolling_rows = read_csv("fact_rolling_forecast_error.csv")
    ...
    holdout = {}
    for r in metrics:
        holdout[r["category_id"]] = (r["test_start"][:10], r["test_end"][:10])

    actual_lookup: dict[tuple[str, str], float] = {}
    for r in search_rows:
        if str(r.get("is_partial", "")).lower() in ("true", "1"):
            continue
        ds = r["week_start"][:10]
        actual_lookup[(r["category_id"], ds)] = num(r, "interest")
    ...
    for r in prophet_rows:
        cid = r["category_id"]
        ds = r["ds"][:10]
        pred_lookup[(cid, ds)] = round(float(num(r, "yhat")), 2)
        dates_by_cat.setdefault(cid, set()).add(ds)
    for cid, ds in actual_lookup:
        dates_by_cat.setdefault(cid, set()).add(ds)

    forecast_series: dict[str, list] = {}
    for cid, dates in dates_by_cat.items():
        t0, t1 = holdout.get(cid, ("2026-02-15", "2026-08-09"))
        pts = []
        for ds in sorted(dates):
            actual = actual_lookup.get((cid, ds))
            predicted = pred_lookup.get((cid, ds))
        ...
        forecast_series[cid] = pts

    rolling_by_cat: dict[str, list] = {}
    for r in rolling_rows:
        cid = r["category_id"]
        mape4 = r.get("rolling_mape_4w", "")
        if mape4 in ("", None):
            continue
        rolling_by_cat.setdefault(cid, []).append(
            {
                "ds": r["ds"][:10],
                "rolling_mape_4w": round(float(num(r, "rolling_mape_4w")), 2),
            }
        )
    for pts in rolling_by_cat.values():
        pts.sort(key=lambda x: x["ds"])

    tier_by_cat = {r["category_id"]: r["uncertainty_tier"] for r in inventory}
        ...
        "forecast_metrics": [
            {
                ...
                "test_start": r.get("test_start", "")[:10],
                "test_end": r.get("test_end", "")[:10],
                "uncertainty_tier": tier_by_cat.get(r["category_id"], "elevated"),
            }
            for r in metrics
        ],
        "inventory_planning": [
    ...
    content = (
        ...
        f"window.AMAZONIQ_DATA = {json.dumps(payload, separators=(',', ':'))};\n"
    )
